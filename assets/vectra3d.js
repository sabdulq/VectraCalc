...
(function () {
  ...
  /* ---------- graph (interactive, draggable 3D plot) ---------- */
  function initGraph(canvas) {
    var renderer = baseRenderer(canvas);
    var s = sizeOf(canvas);
    var camera = new THREE.PerspectiveCamera(48, s.w / s.h, 0.1, 100);
    var scene = new THREE.Scene();
    scene.fog = new THREE.Fog(0x14161b, 9, 20);
    var group = new THREE.Group(); scene.add(group);

    var FUN = [
      { f: function (x, z) { return (x * x - z * z) * 0.10; } },              // saddle
      { f: function (x, z) { return Math.sin(Math.sqrt(x * x + z * z) * 1.7) * 0.75; } }, // ripple
      { f: function (x, z) { return Math.sin(x * 1.1) * Math.cos(z * 1.1) * 0.75; } },    // waves
      { f: function (x, z) { return (x * x + z * z) * 0.07 - 1.1; } }         // bowl
    ];
    var cur = 0, tgtCur = 0, blend = 1;

    var SEG = isMobile ? 40 : 56, SIZE = 6;
    var geo = new THREE.PlaneGeometry(SIZE, SIZE, SEG, SEG); geo.rotateX(-Math.PI / 2);
    var mat = new THREE.MeshBasicMaterial({ color: AMBER, wireframe: true, transparent: true, opacity: 0.6 });
    group.add(new THREE.Mesh(geo, mat));
    var pmat = new THREE.PointsMaterial({ color: AMBER2, size: 0.05, transparent: true, opacity: 0.8 });
    group.add(new THREE.Points(geo, pmat));
    var pos = geo.attributes.position, base = [];
    for (var i = 0; i < pos.count; i++) base.push(pos.getX(i), pos.getZ(i));
    function applySurface() {
      for (var i = 0; i < pos.count; i++) {
        ...
        var y = FUN[cur].f(x, z) * (1 - blend) + FUN[tgtCur].f(x, z) * blend;
        pos.setY(i, y);
      }
      pos.needsUpdate = true;
    }

    var grid = new THREE.GridHelper(SIZE, 12, 0x3a4150, 0x272c36);
    grid.material.transparent = true; grid.material.opacity = 0.4; grid.position.y = -1.6; group.add(grid);
    var dot = new THREE.Mesh(new THREE.SphereGeometry(0.11, 16, 16), new THREE.MeshBasicMaterial({ color: AMBER2 }));
    group.add(dot);

    // orbit controls (manual, no external dependency)
    var theta = 0.7, phi = 1.0, radius = 8.4, dragging = false, lx = 0, ly = 0, idle = 0;
    function updateCam() {
      camera.position.set(
    ...
    canvas.addEventListener('pointerdown', function (e) { dragging = true; lx = e.clientX; ly = e.clientY; idle = 0; canvas.style.cursor = 'grabbing'; try { canvas.setPointerCapture(e.pointerId); } catch (err) {} });
    canvas.addEventListener('pointermove', function (e) {
      if (!dragging) return;
      theta -= (e.clientX - lx) * 0.008; phi -= (e.clientY - ly) * 0.008;
      phi = Math.max(0.35, Math.min(1.45, phi)); lx = e.clientX; ly = e.clientY; idle = 0;
    });
    var stop = function () { dragging = false; canvas.style.cursor = 'grab'; };
    window.addEventListener('pointerup', stop);
    canvas.addEventListener('wheel', function (e) { e.preventDefault(); radius = Math.max(5.5, Math.min(13, radius + (e.deltaY > 0 ? 0.7 : -0.7))); }, { passive: false });

    applySurface(); updateCam();
    function render(t) {
      if (blend < 1) { blend = Math.min(1, blend + 0.06); applySurface(); if (blend >= 1) cur = tgtCur; }
      if (!dragging) { idle += 0.016; if (idle > 1) theta += 0.0018; }
      updateCam();
      var fx = Math.sin(t * 0.55) * 2.2, fz = Math.cos(t * 0.4) * 2.2;
      var fn = FUN[tgtCur].f;
      dot.position.set(fx, fn(fx, fz) + 0.06, fz);
      renderer.render(scene, camera);
    }
    function onResize() { var z = sizeOf(canvas); renderer.setSize(z.w, z.h, false); camera.aspect = z.w / z.h; camera.updateProjectionMatrix(); }

    window.VECTRA = window.VECTRA || {};
    window.VECTRA.setGraph = function (i) {
      i = ((i % FUN.length) + FUN.length) % FUN.length;
      if (i === tgtCur) return;
      cur = tgtCur; tgtCur = i; blend = 0; idle = 0;
    };
    return register(canvas, camera, scene, renderer, render, onResize);
  }

  var INITS = { surface: initSurface, glyphs: initGlyphs, phone: initPhone, morph: initMorph, graph: initGraph };
